...
vit = ViT(input_channels=4*4, class_channels=10, grid_shape=(7,7))
...
print(f'patches_train.shape: {flat_patches_train.shape}')
...
for e in range(epochs):
    ...
    for b in tqdm(range(num_batches)):
        ...
        #             print(f'[grad] {name}')
        #             print(param.grad)
        #         update = learning_rate * param.grad
        #         param -= update
        step += 1
    ...
    for i in range(num_test_steps):
        x = torch.from_numpy(flat_patches_train[i:i+1]).type(torch.FloatTensor) / 255.0 - 0.5
        # x = x.unsqueeze(dim=-1)
        y = model(x)
